...
class OracleCompletionProvider {
      ...
      let isSysFunc;
      if(fullLastWord && this.docDataStore['SYS'][fullLastWord.toUpperCase()]) {
        isSysFunc = true;
      }


			if(lineWords.length < 3 && !isSysFunc) {
				this.destroyDecorationAndSnippet();
				return;
			}
      ...
      container.appendChild(pre);
        ...
        if(!body) {
          ...
          if(this.snippet.packageName != 'SYS') {
        ...
        if(!lineWords) {
          return;
            ...
            for(let func in this.docDataStore['SYS']) {
              if(this.stringStartsWith(func, suggestInfo.prefix)) {
                let snippet = this.docDataStore['SYS'][func];
