...
class OracleCompletionProvider {
      ...
      let isStandardFunc;
      if(fullLastWord && this.docDataStore['STANDARD'][fullLastWord.toUpperCase()]) {
        isStandardFunc = true;
      }


			if(lineWords.length < 3 && !isStandardFunc) {
				this.destroyDecorationAndSnippet();
				return;
			}
      ...
      //console.log(lineWords);
        ...
        if(!isStandardFunc) {
          ...
          procName = fullLastWord.toUpperCase();
        }
      ...
      let container = document.createElement('div');
			...
			pre.classList.add('overlay-autocomplete-oracle');
			pre.innerText = snippet.bodyFullText ? snippet.bodyFullText : '';
      ...
      if(snippet.url) {
        ...
        pre.innerText += '\n';
        a.innerText = 'more..'
        a.href = snippet.url ? snippet.url : undefined;
      ...
      container.appendChild(pre);
		...
		expandSnippet() {
			if(this.snippet && this.snippet.bodyNoDefault) {
        ...
        if(!body) {
          ...
          if(this.snippet.packageName != 'STANDARD') {
            if(atom.config.get('autocomplete-oracle.populateParamsWithPlaceholder') == 'Yes') {
          ...
          snippetHandler.insert(toInsert);
        }
      }
		}
          ...
          return;
				...
				if ((lineWords[wordMaxIndex-1] == '.' && wordMaxIndex >= 2) ||
				    (lineWords[wordMaxIndex] == '.' ) && wordMaxIndex >= 1 ) {
						...
						let doesPackageExist = (this.docDataStore[prevWord.toUpperCase()] != undefined);

						//only get function list if there is an exact match of 1 to the prev token
						if (doesPackageExist){

								let packageObj = this.docDataStore[prevWord.toUpperCase()];


								for (proc in packageObj){
                    let description;
                    ...
                    if(this.docDataStore[packageName] && this.docDataStore[packageName][proc]) {
                      ...
                      descriptionMoreURL = snippet.url;
						...
						for (let pkg in this.dbDataStore.packages){
								if (this.stringStartsWith(pkg, suggestInfo.prefix)){
										allSuggestions.push(this.getPackageSuggestion(pkg, suggestInfo.prefix));
								}
						}

            for(let func in this.docDataStore['STANDARD']) {
              if(this.stringStartsWith(func, suggestInfo.prefix)) {
                let snippet = this.docDataStore['STANDARD'][func];

                allSuggestions.push(this.getProcedureSuggestion(func, suggestInfo.prefix, snippet.descriptionText, snippet.url));
              }
            }
